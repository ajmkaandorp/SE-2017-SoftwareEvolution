...
const SMALLSQL = "data/smallsql.json";
...
function initializeDiagram(allFiles, allClonePairs) {
    ...
    // Setup files and links from the data
    var nodes = cluster.nodes(getFileHierarchy(allFiles, allClonePairs));
    var links = getFileClonePairs(nodes, allClonePairs).filter(link => link.target !== undefined);
    ...
    function getFileClonePairs(nodes, clonePairs) {
        ...
        nodes.forEach(node => { 
            clonePairs.filter(clonepair => node.name === clonepair.origin.file);
            clonePairs.forEach(clonePair => clonePair.clones.forEach(clone => {
                clonePairs.filter(clonePair => node.name == clone.file);

            }));
            
            clonePairs.forEach(clonePair => {
                if(clonePair.origin.file == node.name)
                {
                    clonePair.clones.forEach(clone => { 
                            gatheredClones.push({source: map[node.name], target: map[clone.file]});
                    });
                } else {
                    gatheredClones.push({source: map[node.name], target: map[clonePair.origin.file]});
                }
            });

        });

        return gatheredClones;
    }
